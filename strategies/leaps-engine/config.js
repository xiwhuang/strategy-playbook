...
export default {
  ...
  groups: [
    ...
    {
      id: 'position',
      title: t('Today’s numbers', '今天的数据'),
      description: t('Enter where things stand today to see which rule applies.', '填入今天的情况，查看适用哪条规则。'),
      columns: 4,
    },
  ],
  ...
  inputs: [
    ...
    },
    {
      ...
      kind: 'range',
      group: 'position',
      label: t('Contract delta', '合约 Delta'),
      helper: (r) =>
        t(
          `Check each LEAPS you hold. Harvest above ${r.harvest.delta.value}, buy the dip below ${r.dip.delta.value}`,
          `逐张检查你持有的 LEAPS。高于 ${r.harvest.delta.value} 收割，低于 ${r.dip.delta.value} 逢低加仓`,
        ),
      min: 0.1,
      max: 1,
      step: 0.01,
    ...
    },
    {
      ...
      kind: 'range',
      group: 'position',
      label: t('Days to expiration (DTE)', '剩余到期天数 (DTE)'),
